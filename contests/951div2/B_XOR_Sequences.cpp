...
int firstSetBitPosition(int n) {
    bitset<32> bits(n);  // Assuming 32-bit integers
    for (int i = 0; i < 32; ++i) {
        if (bits.test(i)) {
            return i + 1;  // Return 1-based position
        }
    }
    return 0;  // This line should never be reached if n > 0
}
void solve(int T)
{
    gt(T);
    ...
    w(y);
    int a = x^y;
    int first_set_bit_pos = firstSetBitPosition(a);
    cout << (1<<(first_set_bit_pos-1)) << '\n';
