...
void rec(vi &a, int ind,int sum,int& ans){
    if(ind == sz(a)){
        ans = max(sum,ans);
        ans = max(abs(ans),sum);
    }
    if(a[ind]>0){
        ...
        rec(a,ind+1,abs(sum+a[ind]),ans);
    }
    rec(a,ind+1,sum+a[ind],ans);
...
void solve(int T)
{
    ...
    maxarray[0]=abs(a[0]);
    fr(i,1,sz(a)){
        minarray[i]= min(minarray[i-1]+a[i],maxarray[i-1]+a[i]);
        maxarray[i]= max(abs(minarray[i-1]+a[i]),abs(maxarray[i-1]+a[i]));
    }
    if(sz(minarray)>0 && sz(maxarray)>0){
    cout<<max(abs(minarray.back()),abs(maxarray.back()))<<endl;
    }
    else{
        cout<<0<<endl;
    }
