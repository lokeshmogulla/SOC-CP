...
std::string transformString(const std::string &s, int p) {
    int n = s.size();
    std::string transformed = s;
    
    // Reverse the substring s1s2…sp
    for (int i = 0; i < p / 2; ++i) {
        std::swap(transformed[i], transformed[p - i - 1]);
    }
    
    // Perform a cyclic shift of the string to the left p times
    std::rotate(transformed.begin(), transformed.begin() + p, transformed.end());
...
void solve(int T)
{
    ...
    fr(i,0,n){
        string sd = transformString(s,i+1);
        if(sd==s1 || sd==s2){
            cout<<i+1<<endl;
            return;
        }
