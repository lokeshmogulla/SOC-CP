...
int main() {
    ...
    while (T--) {
        int X, Y;
        cin >> X >> Y; // Read the average speed after 10 hours and the target average speed
        
        // Calculate the total distance covered after 10 hours
        int D = 10 * X;
        
        // Calculate the minimum number of additional hours required
        double n = static_cast<double>(D - 10 * Y) / (Y - 100);
        
        // Use ceil to round up to the nearest integer
        int result = static_cast<int>(ceil(n));
