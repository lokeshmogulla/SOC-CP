...
int twoSum(vi &arr,int target){
    int ans= 0;
    map<int,int>m;
    fr(i,0,sz(arr)){
        if(m.find(((arr[i])+target)%target)!=m.end()){
            ans += m[((arr[i])+target)%target];
            m[((arr[i])+target)%target]++;
        }
        else if(m.find(((arr[i])-target)%target)!=m.end()){
            ans += m[((arr[i])-target)%target];
            m[((arr[i])-target)%target]++;
        }
        else{
