...
void solve(int T)
{
    ...
    int cut = 0;
    int lz=0,lo=0,rz=zeros,ro=ones;
    int mn=MAX;
    while(cut<=n){
        db(cut);db(lz);db(lo);db(ro);db(rz);
        if(lz>=(cut-0+1)/2 && ro>=(n-cut+1)/2){
            mn= (abs(mn-((n)/2.0))>abs(cut-((n)/2.0))) || ((abs(mn-((n)/2.0))==abs(cut-((n)/2.0))) &&  cut<mn)?cut:mn;
        }
        db(mn);
        if(s[cut]=='1'){
            lo++;ro--;
