...
int m,x;
vector<pll>a;
int dp[55][5000000005];
int  rec(int ind, int used){
    if(ind==m){
        return 0;
    }
    if(dp[ind][used]!=-1){
    ...
    int budget= (ind*x - used);
    int ans = rec(ind+1,used);
    if(budget >= a[ind].first){
        ans = max(ans,a[ind].second+rec(ind+1,used + a[ind].first));
    }
