...
vi set1,set2;
void setelements(int start,int end){
    if(start>=end){
        return;
    }
    set1.pb(start);set1.pb(end);
    set2.pb(start+1);set2.pb(end-1);
    setelements(start+2,end-2);
}
void solve(int T)
{
    gt(T);
    w(n);set1.resize(0);set2.resize(0);
    int sum = (n*(n+1))/2;
    if(sum%2){
        cout<<"NO";
        return;
